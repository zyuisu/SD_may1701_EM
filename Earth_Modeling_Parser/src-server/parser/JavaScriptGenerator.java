...
public class JavaScriptGenerator {
	...
	public JavaScriptGenerator(ConvertedSet set) throws IOException {
		...
		strBuff.append(ServerInformation.ARCGIS_PUBLISH_URL);
		strBuff.append(
				"' + region.value + compound.value + 'y' + year.value + selectMonth + '/MapServer'; } var url = new_url; var request; if(window.XMLHttpRequest){ request = new XMLHttpRequest(); } else { request = new ActiveXObject('Microsoft.XMLHTTP'); } request.open('GET', url, false); request.send(); if (request.status !== 200) { alert('The server cannot find that map.'); } else { var lyr = new MapImageLayer({ url: url, opacity: 0.75 }); var map = new Map({ basemap: 'oceans', layers: [lyr] }); view = new SceneView({ container: 'viewDiv', map: map }); var monthTitle = month.options[month.selectedIndex].text; var legendTitle; if (monthTitle == 'Choose a month.') { legendTitle = comp.options[comp.selectedIndex].text + ' ' + year.options[year.selectedIndex].text; } else { legendTitle = month.options[month.selectedIndex].text + ' ' + year.options[year.selectedIndex].text; } view.then(function() { legend = new Legend({ view: view, layerInfos: [{ layer: lyr, title: legendTitle }] }); view.ui.add(legend, 'bottom-right'); }); lyr.then(function() { view.goTo(lyr.fullExtent); }); } }); }); var legendCheck = document.getElementById('legendShow'); legendCheck.addEventListener('click', function(){ require(['esri/Map', 'esri/views/SceneView', 'esri/layers/MapImageLayer', 'esri/widgets/Legend', 'dojo/domReady!', 'dojo/on', 'dojo/dom', ], function(Map, SceneView, MapImageLayer, Legend, domReady, on, dom) { if(legendCheck.value === 'hide'){ view.then(function(){ view.ui.remove(legend); }); legendCheck.value = 'show'; } else { view.then(function(){ view.ui.add(legend, 'bottom-right'); }); legendCheck.value = 'show'; } }); }); }");
